...
#To run as a bsub script: bsub -n 32 -R "span[ptile=2]" R CMD BATCH create_seurat_object.R 
library(Seurat)
...
remove(cond.data) #count data matrix 

immune.anchors <- FindIntegrationAnchors(object.list = cond, dims = 1:20) #Integration between tissues (unused for our manuscript)
remove(cond)
...
saveRDS(immune.combined, "/data/analysis/preeclampsia_2019/analysis/count_matrix/reproduced_seurat_cornelius/placenta_seurat_object_step1.rds")
#Please note that for the ultimate project, we changed our approach to integration & clustering. Look at: Integration_clustering_placenta_step2.R 